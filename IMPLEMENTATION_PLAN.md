...
* **Current Repository Disposition (Main Branch Protection):**
  ...
  - *Disposition:* **`IMPLEMENTATION ACTIVATION PRECONDITION AFTER GATE`**.
    ...
    - However, builders and development agents remain strictly prohibited from beginning Wave 0 / `WP-001` execution, writing implementation code, or opening formal handoff execution until Stage A remote branch protection on `main` is enabled and independently verified on remote by repository administration / DevOps Platform Architect under the **Two-Stage Implementation Activation Bootstrap Protocol (`AMEND-GOV-IR-001` / `ACR-2026-002`)**:
      * **Stage A (Pre-WP-001 Bootstrap):** Stage A remote `main` branch protection must be enabled and independently verified on remote **before `WP-001` implementation begins** (no builder may begin `WP-001`, write implementation code, execute WP changes, or open formal handoff execution until Stage A is verified on remote). Enforces PR requirement (minimum 1 approved review from designated reviewer, builder $\ne$ reviewer), prohibition of direct commits to `main`, and prohibition of force pushes and branch deletions. Automated status check contexts are omitted during Stage A because CI workflows are authored in `WP-002`. Bounded compensating controls apply to `WP-001` (local `npm ci`, local `npm run build`, dependency graph linting, evidence logs, dual review).
      * **Stage B (Post-WP-002 Full Protection):** Immediately after `WP-002` merges and establishes GitHub Actions contexts, branch protection on `main` is updated to require mandatory passing status checks (CI build, lint, typecheck, unit tests, secret scan, sca-scan). Stage B is a hard precondition before `WP-003` or any subsequent domain WP can merge.

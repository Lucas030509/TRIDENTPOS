...
7. **Dual EAAF Agent Review Evidence (Solo Mode):** Under Solo Maintainer Mode (`ADR-010`), GitHub human required approvals are set to 0. Every code-producing WP must obtain PASS evidence from BOTH the assigned Specialist Reviewer Agent and `11_Code_Reviewer` Agent before merge (Builder Agent $\ne$ Reviewer Agent). The reviews must inspect the pinned subject SHA, document Expected vs. Actual evidence, record findings, and produce a formal PASS verdict. These are EAAF agent verification artifacts, not GitHub human approvals.

...
export class PostgresCloudInventoryService implements CloudInventoryCompositionService {
   ...
   * If selectedModifiers are present and no authorized resolver is provided,
   * the event is quarantined in inventory_quarantine_records with zero stock movement.
   */
  public async onKdsOrderProduced(
    ...
    return withTenantTransaction(this.pool, event.organizacionId, async (client) => {
      ...
      for (const item of event.items) {
        ...
        // If custom resolver provided and modifiers present, resolve modifier impacts
        if (customResolver && item.selectedModifiers && item.selectedModifiers.length > 0) {
          for (const mod of item.selectedModifiers) {
            const impact = await customResolver.resolveModifierImpact({
            ...
            if (impact) {
              for (const add of impact.additionalIngredients) {
                ...
                const scaledAdd = multiplyScale4(addGrossScaled, itemScaleFactor);
                rawIngredientItems.push({
                  ingredientId: add.ingredientId,
                  grossScaled: scaledAdd,
                });
              }
              // Removed ingredients can be handled by subtracting if needed in test resolver
            }
          }
        }
      }
  ...
  public async replayQuarantinedDepletion(
    ...
    return withTenantTransaction(this.pool, organizationId, async (client) => {
      ...
      // Run depletion through normal path with the resolver
      const depletionResult = await this.onKdsOrderProduced(eventPayload, resolver);

/**
 * TRIDENTPOS KDS Edge SQLite Schema DDL (WP-015, ACR-2026-016)
 * Data Objects per IMPLEMENTATION_PLAN.md WP-015 entry: kds_estaciones,
 * kds_tickets, kds_ticket_partidas, impresoras_red.
 *
 * Conforms strictly to DATA_MODEL.md and ADR-012:
 * - kds_tickets.preparation_time_minutes is INTEGER NULL CHECK (>= 0).
 * - kds_ticket_partidas.quantity is INTEGER NOT NULL representing scale-4 fixed-point (factor 10,000).
 *
...
CREATE TABLE IF NOT EXISTS kds_tickets (
    ...
    preparation_time_minutes INTEGER NULL CHECK (preparation_time_minutes IS NULL OR preparation_time_minutes >= 0)
);

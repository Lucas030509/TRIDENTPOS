...
-- Ownership: warehouses, ingredients, recipes, recipe_items belong to Inventory.
-- Platform Core owns products and categories (WP-016B). WP-017 consumes products.
-- ============================================================================
...
CREATE POLICY tenant_isolation_policy ON recipe_items
    FOR ALL
    USING (organization_id = current_app_org_id())
    WITH CHECK (organization_id = current_app_org_id());

-- Down
DROP POLICY IF EXISTS tenant_isolation_policy ON recipe_items;
